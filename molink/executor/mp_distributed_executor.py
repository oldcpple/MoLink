...
from molink.executor.request_tracker import RequestTracker
from molink.executor.request_tracker import ServerFailureError
from molink.executor.request_tracker import MoLinkEvent
import os

mp.set_start_method('spawn', force=True)

class MultiprocessingDeliver(mp.Process):
    def __init__(self, initial_peer, pipeline_parallel_size, error_event):
        super().__init__()
    ...
    async def mp_async_transmit(
        self,
        bytes_emr,
        grpc_intermediate_tensors,
        grpc_metadata,
        virtual_engine,
        next_server
    ):
        try:
            if self.preset_next_server != next_server:
                ...
                # print(f'self.present_next_server == {self.preset_next_server}, next_server == {next_server}', flush=True)
            
            # transmit activation to next server
            ...
            raise ServerFailureError(f"Detected Server {next_server} FAILED!")
        
        except Exception as e:
            print(f"Meet an {get_color('UNEXPECTED', 'red')} error: {e}", flush=True)
            traceback.print_exc()
    ...
    async def recovery_next_server(self, intermediate_tensors_cpu, execute_model_req, grpc_metadata, virtual_engine, next_server):
        bytes_emr, grpc_intermediate_tensors = self.mp_serialize_intermediate_tensors(intermediate_tensors_cpu,\
                                                                                                execute_model_req)
                    
        print('iteration in recovery', flush=True)
        ...
        try:
            ...
            f"{get_color('DISTRIBUTED SERVICE INFO', 'red')}: Meet an unexpected error when recovery! System TERMINATED!"
            traceback.print_exc()
...
class MolinkMultiprocessingDistributedExecutor(MultiprocessingDistributedExecutor):
    ...
    def __init__(
        ...
        self.error_event = MoLinkEvent()
        self.new_server_event = MoLinkEvent()
        self.tracker = RequestTracker(self.parallel_config.pipeline_parallel_size)

        self._init_executor()

        asyncio.create_task(self.handle_server_failure())
    ...
    def _init_executor(self) -> None:
        ...
        if self.use_dht:
            ...
            print(f"{get_color('DISTRIBUTED SERVICE INFO', 'green')}: MoLink gRPC server works at {grpc_info}, ")
            print(f"{get_color('DISTRIBUTED SERVICE INFO', 'green')}: MoLink DHT server works at {dht_info}")
            print(f"{get_color('DISTRIBUTED SERVICE INFO', 'green')}: If this is the first node of the swarm, you can copy the DHT INFO as the initial peer of following nodes")
            ...
            print(f"{get_color('DISTRIBUTED SERVICE INFO', 'green')}: MoLink gRPC server works at {grpc_info}, ")
            print(f"{get_color('DISTRIBUTED SERVICE INFO', 'green')}: If this is the first node of the swarm, you can copy the GRPC INFO as the initial peer of following nodes")
        ...
        self.mp_deliver = MultiprocessingDeliver(initial_peer, self.parallel_config.pipeline_parallel_size, self.error_event)
    ...
    async def executing_head_server(
        ...
        try:
            ...
            # put into deliver queue
            self.tracker.update(copy.deepcopy(intermediate_tensors_cpu), copy.deepcopy(execute_model_req), grpc_metadata, virtual_engine)
    ...
    def update_server_list(self, replace: bool, new_node: comm_pb2.NodeInfo):
        # 从 comm_handler 获取最新的节点信息
        grpc_metadata = get_grpc_metadata(f'{self.ip}:{self.grpc_port}', self.comm_handler.node_info_dict)
        server_list = grpc_metadata.get('server_list', [])
        print(f"here: create_stubs: {server_list}", flush=True)
        self.create_stubs(server_list)
        self.preset_next_server = self.preset_server_list[((self.preset_server_list.index(f"{self.ip}:50051") + 1) % len(self.preset_server_list))]

        # 是否有节点被替换?
        if replace:
            # 替换 --> 判断是否进入故障恢复流程
            self.new_server_event.set(f"get new server:{new_node.ip} [{new_node.start_layer}:{new_node.end_layer}]")
            print(f'pipeline update: {self.preset_server_list}', flush=True)
        else:
            # 无替换 --> 流水线初始化过程
            print(f'pipeline initial update: {self.preset_server_list}', flush=True)

    async def handle_server_failure(self):
        '''NOTE
        如果长时间没恢复出现asyncio.exceptions.TimeoutError
        修改/usr/local/lib/python3.10/dist-packages/vllm/engine/async_llm_engine.py第31行
        ENGINE_ITERATION_TIMEOUT_S = envs.VLLM_ENGINE_ITERATION_TIMEOUT_S --> ENGINE_ITERATION_TIMEOUT_S = None
        '''
        ...
        while True:
            ...
            if self.error_event.is_set():
                ...
                print(f"{get_color('DISTRIBUTED SERVICE INFO', 'red')}: {self.error_event.get_message()}", flush=True)
                print(f"{get_color('DISTRIBUTED SERVICE INFO', 'yellow')}: Waiting for a new server.", flush=True)
                # todo 未来增加head节点通知k8s中的master，要求分配新pod
                self.new_server_event.clear()

                wait_time = 0
                while(True):
                    await asyncio.sleep(1)
                    wait_time += 1
                    if self.new_server_event.is_set():
                        print(f"{get_color('DISTRIBUTED SERVICE INFO', 'yellow')}: {self.new_server_event.get_message()}", flush=True)
                        print(f"{get_color('DISTRIBUTED SERVICE INFO', 'yellow')}: Start to recover!", flush=True)
                        # 真实恢复流程
                        print(f"{get_color('DISTRIBUTED SERVICE INFO', 'yellow')}: Get activation from cache...", flush=True)
                        pipeline_info = self.tracker.get_data()
                        print(f"{get_color('DISTRIBUTED SERVICE INFO', 'yellow')}: Transmit activation to next server...", flush=True)
                        for pipe in pipeline_info:
                            await self.mp_deliver.recovery_next_server(pipe['intermediate_tensors_cpu'],\
                                                                      pipe['execute_model_req'],\
                                                                      pipe['grpc_metadata'],\
                                                                      pipe['virtual_engine'], self.preset_next_server)

                            print(f"self.stub_list == {self.stub_list}")
                            next_server_stub = self.stub_list[1]
                            trigger_request = comm_pb2.GrpcTriggerRequest(virtual_engine = pipe['virtual_engine'])
                            try:
                                await call_stub(next_server_stub, trigger_request)
                            except grpc.aio.AioRpcError as e:
                                if e.code() == grpc.StatusCode.CANCELLED:
                                    print("[recovery] trigger cancelled by peer", flush=True)
                                else:
                                    print(f"[recovery] trigger rpc error: {e.code().name} {e.details()}", flush=True)
                                    raise

                            print(f"{get_color('DISTRIBUTED SERVICE INFO', 'green')}: Recovery success! Continue to calculate!", flush=True)

                        self.new_server_event.clear()
                        self.error_event.clear()
                        break
                    elif wait_time % 10 == 0:
                        print(f"{get_color('DISTRIBUTED SERVICE INFO', 'yellow')}: Keep waiting...({wait_time}s)", flush=True)
...
def get_color(message: str, color: str):
    if color == 'red':
