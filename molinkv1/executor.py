...
class MolinkExecutor(MultiprocExecutor):
    ...
    # MoLink's execute_model returns complete ModelRunnerOutput (including sampling)
    # so sample_tokens should not be called separately
    returns_complete_output: bool = True
    ...
    def __init__(self, 
        ...
        if self.molink_config is None:
            ...
            from vllm.config.molink import MolinkConfig
        ...
        # Micro-batch (virtual engine) management
        self.max_batch_num = self.molink_config.max_batch_num
        self._batch_futures: Dict[int, Future] = {}  # virtual_engine -> Future
        self._batch_scheduler_outputs: Dict[int, "MolinkSchedulerOutput"] = {}  # virtual_engine -> MolinkSchedulerOutput
        self._active_batches: int = 0
        self._batch_completion_events: Dict[int, asyncio.Event] = {}
    ...
    def max_concurrent_batches(self) -> int:
        """Return the maximum number of concurrent batches (micro-batches)."""
        if self.molink_config.enable_micro_batch:
    ...
    def _init_molink(self) -> None:
        ...
        future = asyncio.run_coroutine_threadsafe(
            self._init_grpc_server(start_layer, end_layer),
            self._event_loop
        )
        ...
        logger.info(f"MoLink executor initialized. "
    ...
    def execute_model(
        ...
        For head node: orchestrates the distributed pipeline with micro-batch support.
        For worker node: should not be called directly (uses service)
        
        The micro-batch design allows multiple batches to be processed concurrently,
        overlapping computation on one batch with communication for another batch.
        This reduces pipeline bubbles and improves throughput.
        
        Args:
            scheduler_output: The scheduler output with virtual_engine set.
        ...
        # logger.info(f"[MoLink][VE{virtual_engine}] execute_model called, non_block={non_block}")
        
        # Head node: orchestrate the pipeline
        if non_block:
            # Create a wrapper Future that will be completed by the async execution
            future = Future()
            
            async def execute_and_set_result():
                try:
                    result = await self._execute_model_async(scheduler_output)
            ...
            asyncio.run_coroutine_threadsafe(
                execute_and_set_result(),
                self._event_loop
            )
            return future
        else:
            async_future = asyncio.run_coroutine_threadsafe(
                self._execute_model_async(scheduler_output),
                self._event_loop
            )
    ...
    async def _execute_model_async(
        ...
        Supports micro-batch processing where multiple virtual engines can
        execute concurrently to overlap compute and communication.
        ...
        try:
            ...
            scheduler_output_bytes = cloudpickle.dumps(scheduler_output, protocol=pickle.HIGHEST_PROTOCOL)
            # logger.info(f"[MoLink][VE{virtual_engine}] Serialized scheduler output: {len(scheduler_output_bytes)} bytes")
            # logger.info(f"[MoLink][VE{virtual_engine}] Scheduler output: new_reqs={len(scheduler_output.scheduled_new_reqs)}, "
            ...
            if scheduler_output.total_num_scheduled_tokens == 0:
                # logger.info(f"[MoLink][VE{virtual_engine}] No tokens to schedule, skipping execution")
                ...
                #     logger.info(f"[MoLink][VE{virtual_engine}] Request(s) finished: {scheduler_output.finished_req_ids}")
                # logger.info(f"[MoLink][VE{virtual_engine}] Returning empty ModelRunnerOutput")
            ...
            if isinstance(output_bytes, bytes):
                ...
                # logger.info(f"[MoLink][VE{virtual_engine}] Deserialized result: {type(result).__name__}")
            else:
                result = output_bytes
    ...
    async def _executing_head_server(
        ...
        try:
            ...
            async with self.pp_lock:
                # logger.info(f"[MoLink][VE{virtual_engine}][HEAD] Calling _driver_exec_model")
                output = await self._driver_exec_model(scheduler_output)
                # logger.info(f"[MoLink][VE{virtual_engine}][HEAD] _driver_exec_model completed, output type: {type(output).__name__}")
            
            server_list = grpc_metadata.get('server_list', [])
            
            # If this is the only node, put result directly in output queue
            if len(server_list) <= 1:
                # logger.info(f"[MoLink][VE{virtual_engine}][HEAD] Single node, putting result in output queue")
                ...
                # logger.info(f"[MoLink][VE{virtual_engine}][HEAD] Result placed in output queue")
                return
            
            # Multi-node case: extract and send intermediate tensors
            next_server = server_list[1]
            # logger.info(f"[MoLink][VE{virtual_engine}][HEAD] Multi-node, next server: {next_server}")
            
            # Get intermediate tensors
            if isinstance(output, IntermediateTensors):
                intermediate_tensors = output.tensors
                # logger.info(f"[MoLink][VE{virtual_engine}][HEAD] Got IntermediateTensors with keys: {list(intermediate_tensors.keys())}")
            elif hasattr(output, 'tensors'):
                intermediate_tensors = output.tensors
                # logger.info(f"[MoLink][VE{virtual_engine}][HEAD] Got tensors from output.tensors with keys: {list(intermediate_tensors.keys())}")
            else:
                ...
                # logger.info(f"[MoLink][VE{virtual_engine}][HEAD] Got final output (non-intermediate), type: {type(output)}")
                # Serialize using cloudpickle for consistency
                output_bytes = cloudpickle.dumps(output)
                await self.molink_service.output_queue[virtual_engine].put(output_bytes)
                # logger.info(f"[MoLink][VE{virtual_engine}][HEAD] Final output placed in queue")
                return
            
            # Deliver to next node (async in background process)
            # logger.info(f"[MoLink][VE{virtual_engine}][HEAD] Delivering {len(intermediate_tensors)} tensors to {next_server}")
            ...
            # logger.info(f"[MoLink][VE{virtual_engine}][HEAD] Delivery initiated (async)")
    ...
    async def _driver_exec_model(
        ...
        # logger.info(f"[MoLink][VE{virtual_engine}][DRIVER] Calling parent execute_model")
        ...
        if intermediate_tensors is not None:
            # logger.info(f"[MoLink][VE{virtual_engine}][DRIVER] Have intermediate tensors with {len(intermediate_tensors.tensors)} items")
            ...
            # logger.info(f"[MoLink][VE{virtual_engine}][DRIVER] Setting intermediate tensors on workers")
        ...
        # logger.info(f"[MoLink][VE{virtual_engine}][DRIVER] Waiting for execution to complete")
        loop = asyncio.get_running_loop()
        result = await loop.run_in_executor(None, future.result)
        
        # logger.info(f"[MoLink][VE{virtual_engine}][DRIVER] Execution completed, result type: {type(result).__name__}")
    ...
    async def execute_worker_step(
        ...
        """
        try:
            # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] Starting execute_worker_step")
            
            # Deserialize scheduler output using cloudpickle
            scheduler_output = cloudpickle.loads(scheduler_output_bytes)
            # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] Deserialized scheduler output")
            # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] Scheduler output: new_reqs={len(scheduler_output.scheduled_new_reqs)}, "
            #            f"cached_reqs={len(scheduler_output.scheduled_cached_reqs.req_ids)}, "
            #            f"total_tokens={scheduler_output.total_num_scheduled_tokens}")
            ...
            # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] Received {len(intermediate_tensors.tensors)} intermediate tensors")
            
            # Execute on local workers
            # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] Calling _driver_exec_model")
            output = await self._driver_exec_model(
                scheduler_output,
                intermediate_tensors
            )
            # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] _driver_exec_model completed, output type: {type(output).__name__}")
            
            # If output is None, we need to call sample_tokens (for last stage)
            if output is None:
                # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] Output is None, calling sample_tokens")
                ...
                # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] sample_tokens completed, output type: {type(output).__name__}")
            
            server_list = grpc_metadata.get('server_list', [])
            # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] Pipeline has {len(server_list)} nodes")
            
            # Find my position in pipeline
            try:
                my_idx = server_list.index(self.grpc_address)
                # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] My position in pipeline: {my_idx}")
            except ValueError:
                logger.error(f"[MoLink][VE{virtual_engine}][WORKER_STEP] Node {self.grpc_address} not found in server list: {server_list}")
                return output
            
            is_last_stage = my_idx == len(server_list) - 1
            # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] Is last stage: {is_last_stage}")
            
            if is_last_stage:
                # Send final output back to head node
                head_server = grpc_metadata.get('head')
                # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] Last stage, sending final output to head: {head_server}")
                ...
                if isinstance(output, ModelRunnerOutput):
                    ...
                    # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] Serialized ModelRunnerOutput: {len(output_bytes)} bytes")
                else:
                    logger.error(f"[MoLink][VE{virtual_engine}][WORKER_STEP] ERROR: Last stage output is not ModelRunnerOutput, type: {type(output)}")
                ...
                # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] Final output delivery initiated")
            else:
                # Send intermediate tensors to next node
                next_server = server_list[my_idx + 1]
                # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] Intermediate stage, next server: {next_server}")
                
                if isinstance(output, IntermediateTensors):
                    intermediate = output.tensors
                    # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] Got IntermediateTensors with {len(intermediate)} items")
                elif hasattr(output, 'tensors'):
                    intermediate = output.tensors
                    # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] Got tensors from output with {len(intermediate)} items")
                else:
                    intermediate = {'hidden_states': output}
                    # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] Wrapping output in hidden_states")
                
                # Serialize scheduler output for forwarding
                # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] Delivering {len(intermediate)} tensors to {next_server}")
                self.delivery_manager.deliver_to_next(
                    intermediate,
                    scheduler_output_bytes,
                    grpc_metadata,
                ...
                # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] Intermediate tensor delivery initiated")
            
            # logger.info(f"[MoLink][VE{virtual_engine}][WORKER_STEP] execute_worker_step completed")
            return output
